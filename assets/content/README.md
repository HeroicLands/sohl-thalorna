...
```markdown
See [[creature-grkrahk|Grukar-ahk]] and [[doc-pelwarpepl|the Pelwar People]].
```
...
into that repository's `assets/manifests/`). See `utils/kb-manifest.mjs` for the
format, and note the consequence: once every package's manifest is present, an
address that resolves in none of them stops being tolerated and fails the build.
