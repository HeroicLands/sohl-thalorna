---
tags:
  - mountain
  - inland
  - coastal
description: "The great eastern range of Aelwyth — running west to east across Stormveld's southern border, then south down the coast to break into the foothills above Dúnavarre, and holding the island's Khazári."
type: place
subType: feature
data:
  demonym: null
  lore: []
  parents:
    - aelwyth
  population: null
name:
  full: The Ironfells
  aliases: []
shortcode: ironfells
packFolder: aelwyth
---

# The Ironfells

The **Ironfells** are the spine of eastern Aelwyth. They rise along the southern border of
[[affiliation-jrldmstrmvld|Stormveld]], run eastward to the coast, and then turn **south down the
eastern seaboard**, falling away by degrees into the foothills and at last into the lowlands of
[[affiliation-kingdmdnvr|Dúnavarre]].

They are not high by continental standards and they do not need to be. Aelwyth is a **cool, wet
island**, and the Ironfells are steep, sodden, thickly weathered and snow-covered for a third of the
year — country that is crossed at two places and otherwise gone round.

## The Two Passes

Only **two passes** carry traffic between the Stormveld plain and Aldorath, and both run through deep
wilderness on either approach. They are the reason the Jarldom's expansion southward has never happened
and the reason Aldorath has never seriously attempted to take the north-east back: an army can get
through, but only in single file, only in summer, and only if nobody is holding the far end.

The passes are watched from the south at [[place-caerwynd|Caerwynd]] and from the north by whichever
of the jarl's men has currently displeased him.

## The Rain-Shadow

The range's other consequence is agricultural and out of all proportion to its size. The prevailing
weather comes off the western ocean, wet and cold; by the time it has crossed the island and struck the
Ironfells it has little left to give, and the **western slopes and the country in their lee are the
driest and warmest ground on Aelwyth**.

That is why [[place-sunnerslade|Sunnerslade]] grows vines at a latitude that has no business growing
them, and why Aldorath's wine is worth carrying to the mainland.

## The Khazári

Somewhere in the eastern heights, where the range meets the sea cliffs, is
**[[place-vorgald|Vorgald]]** — a hold of the [[lore-flkkhazar|Khazári]], the Deep Folk. That it exists
is known. Very little else is.

## See Also

- [[place-aelwyth|Aelwyth]] · [[place-vorgald|Vorgald]] · [[place-sunderfells|The Sunderfells]]
